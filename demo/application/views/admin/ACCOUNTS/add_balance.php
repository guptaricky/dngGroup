...
GetFundTransfer();
	...
	$('#checkout-form').on('submit',function(e){//bind event on form submit.
		...
		if(transfer_from=='' || transfer_to==''){
			...
			$(".btn").button('loading');
		 $.ajax({
			 url:"<?php echo base_url('admin/ACCOUNTS/add_balance_to_account'); ?>",
			 type:"post",
		...
		var content ='';	
		content +='<table class="table table-bordered"><thead><tr><th>Account</th><th>Transfer Amount</th><th>Date</th><th>Purpose</th><th>Remark</th><th>Action</th></tr></thead><tbody>';			
		$.getJSON('<?php echo base_url('admin/ACCOUNTS/get_balance_to_account'); ?>','', function(data){
					$.each(data.list, function (k, v) {
					  content +='<tr><td>'+ v.transfer_to +'</td><td>'+ v.transfer_amt +'</td><td>'+ v.transfer_date +'</td><td>'+ v.transfer_perpose +'</td><td>'+ v.transfer_remark +'</td><td><button class="btn btn-info btn-xs" title="Edit" onclick="EditFundTransfer('+ v.transfer_id +')"><i class="fa fa-edit"></i></button>&nbsp;<button class="btn btn-danger btn-xs" title="Delete" onclick="DeleteFundTransfer('+ v.transfer_id +')" ><i class="fa fa-remove"></i></button></td></tr>';
					});					
					content +='</tbody></table>';	
		...
		$.post('<?php echo base_url('admin/ACCOUNTS/edit_balance_to_account'); ?>', {'id':id}, function(response){
		...
		$.ajax({  
				type: "POST",
				url: "<?php echo base_url('admin/ACCOUNTS/delete_balance_to_account'); ?>",
				data: {'id':id},
